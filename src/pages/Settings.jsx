...
function Settings() {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.put(
        `https://lms-production-f51d.up.railway.app/api/auth/update-user/${userData.id}`,
        userData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDelete = async () => {
    try {
      ...
      const token = sessionStorage.getItem("authToken");
      await axios.delete(`https://lms-production-f51d.up.railway.app/api/auth/delete-user/${userData.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
