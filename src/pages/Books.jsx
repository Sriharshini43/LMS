...
function Books() {
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      const borrowStatusPromises = booksData.map(async (book) => {
        const borrowResponse = await axios.get(`http://localhost:3000/api/borrow/status/${userId}/${book.id}`);
        return {
          ...book,
  ...
  const checkBorrowLimit = async () => {
    ...
    try {
      const borrowedResponse = await axios.get(`http://localhost:3000/api/borrow/user/${userId}`);
